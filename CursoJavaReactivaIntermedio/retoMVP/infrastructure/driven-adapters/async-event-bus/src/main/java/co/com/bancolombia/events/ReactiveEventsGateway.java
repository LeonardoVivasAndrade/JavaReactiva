...
public class ReactiveEventsGateway implements EventsGateway {
    ...
    public static final String MOVEMENTS_EVENT_UPLOAD = "movements.event.upload";
    ...
    @Override
    public Mono<Void> boxMovementsUploadedEvent(MovementUploadReport event) {
        log.log(Level.INFO, "Sending domain event: {0}: {1}", new String[]{MOVEMENTS_EVENT_UPLOAD, event.toString()});
        return from(domainEventBus.emit(new DomainEvent<>(MOVEMENTS_EVENT_UPLOAD, UUID.randomUUID().toString(), event)));
    }
}
